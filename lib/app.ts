...
function launchBot(server): void {
  function getPost(postType: PostType, author?: Author): Post {
    ...
    for (let i = 0; i < Posts.length; i++) {
      if (postType === PostType.Enclosure && Posts[i].enclosure != null && Posts[i].enclosure.url != null) {
        ...
        postType === PostType.Author &&
        author === Author.Ahern &&
        Posts[i].author.toLowerCase().indexOf('ahern') !== -1
      ) {
        latest = Posts[i]
        break
      } else if (
        postType === PostType.Author &&
        author === Author.Szul &&
        Posts[i].author.toLowerCase().indexOf('szul') !== -1
      ) {
        latest = Posts[i]
        break
      }
  ...
  intents.matches(/from bill|by bill/i, [
    (session) => {
      ...
      session.send(
        'The latest from Bill on #codepunk is titled "%s" and was published on %s',
        latest.title,
        latest.pubdate.toDateString()
      )
      session.send(
        new builder.Message(session).addAttachment(
          createThumbnailCard(session, latest.title, latest.link, latest.image.url)
        )
      )
    }
  ])
  ...
  intents.matches(/from michael|by michael/i, [
    (session) => {
      ...
      session.send(
        'The latest from Michael on #codepunk is titled "%s" and was published on %s',
        latest.title,
        latest.pubdate.toDateString()
      )
      session.send(
  ...
  intents.matches(/latest podcast|latest episode|newest podcast|newest episode/i, [
    (session) => {
      ...
      session.send(
        'The latest podcast episode on #codepunk is titled "%s" and was published on %s',
        latest.title,
        latest.pubdate.toDateString()
      )
      session.send(
        new builder.Message(session).addAttachment(
          createAudioCard(session, latest.title, latest.link, latest.enclosure.url, latest.image.url)
        )
      )
    }
  ])

  intents.matches(/latest article|last article|latest blog post|last blog post|newest blog post|newest article/i, [
    //You can make all of these more regex-y
    (session) => {
      let latest: Post = getPost(PostType.Article)
      session.send(
        'The latest blog post on #codepunk is titled "%s" and was published on %s',
        latest.title,
        latest.pubdate.toDateString()
      )
      session.send(
  ...
  intents.matches(/^what's new|^what's up|^whats new|^whats up/i, [
    (session) => {
      ...
      let postType =
          ...
          latest.enclosure == null ? 'an article' :
          'a podcast'
      session.send(
        'The latest on #codepunk is %s titled "%s" and was published on %s',
        postType,
        latest.title,
        latest.pubdate.toDateString()
      )
...
function createThumbnailCard(session: any, title: string, linkUrl: string, imageUrl: string): any {
  return new builder.ThumbnailCard(session)
    ...
    .buttons([ builder.CardAction.openUrl(session, linkUrl.replace('http://', 'https://'), 'Read more...') ])
}
